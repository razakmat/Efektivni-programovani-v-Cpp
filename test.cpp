...
public:
    X() : i_(-1) { constructed++; default_constructed++; alive++; }
    X(int i) : i_(i) { constructed++; converting_constructed++; alive++; }
    X(const X& other) : i_(other.i_) { constructed++; copy_constructed++; alive++; }
    X(X&& other) : i_(other.i_) { other.i_ = -1; constructed++; move_constructed++; alive++; }
    ~X() { destructed++; alive--; }
